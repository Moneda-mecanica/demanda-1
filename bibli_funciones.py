...
def cargar_info_raw_demanda(ruta_raw):
    ...
    cont = 0
    tabla_1_activa = pd.DataFrame()
    for archivo in os.listdir(ruta_raw):
        if archivo.endswith('.raw'):
            nombre_archivo = os.path.join(ruta_raw, archivo)
            
            with open(nombre_archivo) as raw:
    ...
    df_total['Activa'] = pd.to_numeric(df_total['Activa'], errors='coerce')  
    tabla_1_activa = df_total.pivot_table(index=["fecha_hora"], columns="Nombre", values="Activa", aggfunc="sum") 
    ...
    #df_n.set_index('fecha_hora', inplace=True)
    #tabla_1_activa.set_index('fecha_hora', inplace=True)
    ...
    return print('cargado existosamente')
    

   
def leer_archivo_raw_demanda(ruta_raw):
    df_total = pd.DataFrame(columns= ['Fecha', 'Nombre', 'Codigo', 'Activa', 'Reactiva'])
    cont = 0
    for archivo in os.listdir(ruta_raw):
        if archivo.endswith('.raw'):
    ...
    return tabla_1_activa, tabla_activa, fecha_1
...
def cargar_datos_excel( fecha):
    ...
    if dia_semana in ['Tuesday', 'Wednesday', 'Thursday', 'Friday']:
        ...
        fecha_inicio = fecha_referencia - timedelta(weeks=6)
...
def ejecutar_final(ruta_raw, ejecucion_demanda_parte_2, leer_archivo_raw_demanda, cargar_datos_excel):
    
    #ruta_dem_historica_a    = r"C:\Users\lportilla\Downloads\PSO\PROYECTO\Filtro_Por_DiaSemana\ACTIVA_DEM.xlsx"

    df_curva_analisis_dem_a, tabla_activa, fecha = leer_archivo_raw_demanda(ruta_raw)
    df_curva_historicas_dem_a   = cargar_datos_excel(fecha)
    ...
    #usuarios_filtrados = usuarios.dropna()
    usuario_3_a = list()

    fecha = pd.to_datetime(fecha)

    # Calculos de la demanda
    tabla_datos_desviaciones_final_dem_a, tabla_datos_totales_final_dem_a, datos_frecuencia_desviaciones_dem_a, usuario_atipico, data_atipico, df_curva_analisis = ejecucion_demanda_parte_2(fecha, df_curva_historicas_dem_a, df_curva_analisis_dem_a, usuarios_filtrados, usuario_3_a, contar_usuarios)
    ...
    return tabla_datos_desviaciones_final_dem_a, tabla_datos_totales_final_dem_a, datos_frecuencia_desviaciones_dem_a, usuario_atipico, data_atipico, df_curva_analisis, tabla_dem_total_a
...
def tab_graficos_dem_a(tab_linea, tab_tabla, app_color,tabla_formato_encabezado, tabla_formato_celdas,  df_filtrado_dem, df_grafico_histograma, df_tabla_desv, df_tabla_total_datos, nombre_seleccionado_dem_a):
     ...
     if tab_linea == 'grafico-dem-activa-por-usuario':
         ...
         return  grafico_linea, grafico_tabla
...
def Aplicativo(app, url, ejecucion_demanda_parte_2, dem_total):   
    ...
    app.layout = html.Div([
       ...
       dbc.Row([
       html.Div([
       html.H6("SUBIR INFORMACIÓN A BD:", className="app__header", style={'marginLeft': '10px','color':'#00ffff'}),
       ],
       className="app__header__desc"),

       
       dcc.Loading( id="loading-1", type="default",
       children = [dcc.Input(id='file-path-cargar', type='text', placeholder='Ingresa la ruta del fichero', style={'marginLeft': '300px','align-items': 'center','width': '800px'}),
       html.Button('Ejecutar-cargar', id='execute-button-cargar', n_clicks=0, style={'width': '200px','marginLeft': '15px', 'color':'#00ffff', 'borderTop': '1px solid #00ffff', 'borderBottom': '1px solid #00ffff', 'borderLeft': '1px solid #00ffff', 'borderRight': '1px solid #00ffff'}),
       #dcc.Store(id='informacion-cargar'),
       ...
       dbc.Row([
       html.Div([
       html.H6("EXTRAER INFORMACION DE CARPETA:", className="app__header", style={'marginLeft': '10px','color':'#00ffff'}),
       ],
       className="app__header__desc"),
       
       dcc.Loading( id="loading-2", type="default",
       children =html.Div([
       dcc.Input(id='file-path', type='text', placeholder='Ingresa la ruta del fichero', style={'marginLeft': '300px','align-items': 'center','width': '800px'}),
       html.Button('Ejecutar-extraer', id='execute-button', n_clicks=0, style={'width': '200px','marginLeft': '15px', 'color':'#00ffff', 'borderTop': '1px solid #00ffff', 'borderBottom': '1px solid #00ffff', 'borderLeft': '1px solid #00ffff', 'borderRight': '1px solid #00ffff'}),
           ...
           [
            ...
            html.Div(           
                   [html.Div([
                           ...
                           dcc.Store(id='indice-actual', data=1),
                           ])]),]),
            ...
            html.Br(),
                           ...
                           html.Div([dcc.Tabs(id="tabs_tabla_dem_activa", value='tabla_1_value_dem_activa', className='dash-tabs', 
                           children=[
                               dcc.Tab(label='Tabla De Desviaciones', value='tabla_1_value_dem_activa',style = tab_boton_formato_sin_seleccion , selected_style=tab_boton_formato_seleccion),
                               dcc.Tab(label='Tabla de Datos', value='tabla_2_value_dem_activa',style = tab_boton_formato_sin_seleccion , selected_style=tab_boton_formato_seleccion),
                               ] ),
                           html.Br(),
                           html.Div(id='tabs_content_tabla_dem_activa'),],),
    ...
    @app.callback(
        ...
        Input('execute-button-cargar', 'n_clicks'),
        Input('file-path-cargar', 'value'))
        
    def cargar(n_clicks, ruta):
        ctx = dash.callback_context
        if not ctx.triggered:
            return no_update
        button_id = ctx.triggered[0]['prop_id'].split('.')[0]

        if button_id == 'execute-button-cargar':
          cargar_info_raw_demanda(ruta) 
          time.sleep(1)
          return None
    ...
    def extraer(n_clicks, ruta):
        ctx = dash.callback_context
        if not ctx.triggered:
            return no_update, no_update
        button_id = ctx.triggered[0]['prop_id'].split('.')[0]

        if button_id == 'execute-button':
          
          ruta_raw = ruta
          tabla_datos_desviaciones_final_dem_a, tabla_datos_totales_final_dem_a, datos_frecuencia_desviaciones_dem_a, usuario_atipico, data_atipico, df_curva_analisis, tabla_dem_total_a = ejecutar_final(ruta_raw, ejecucion_demanda_parte_2, leer_archivo_raw_demanda, cargar_datos_excel)
    ...
    @app.callback(
        [Output('lista_desplegable_dem_activa', 'value',  allow_duplicate=True),
        Output('indice-actual', 'data'),
         Output('informacion-1', 'data', allow_duplicate=True)],
        [Input('continuar-boton', 'n_clicks'),
         Input('retroceder-boton', 'n_clicks')],
        [State('informacion-1', 'data')],
        [Input('indice-actual', 'data')]
    )
    def cambiar_indice_1(n_clicks_continuar, n_clicks_retroceder, data, current_index):
        if data is not None:
            ctx = dash.callback_context
            if not ctx.triggered:
                return data['option_usuario_3_a'][current_index]['value'], current_index, data
            button_id = ctx.triggered[0]['prop_id'].split('.')[0]
    
            if button_id == 'continuar-boton':
                if current_index < len(data['option_usuario_3_a']) - 1:
                    return data['option_usuario_3_a'][current_index + 1]['value'],current_index + 1, data
                
            elif button_id == 'retroceder-boton':
                if current_index > 0:
                    return data['option_usuario_3_a'][current_index - 1]['value'], current_index - 1, data
    ...
    @app.callback(
                  ...
                  Output('tabs_content_tabla_dem_activa', 'children')],
    ...
    def actualizar_dashboard_dem_a(nombre_seleccionado_dem_a, data, tab_linea_dem_a, tab_dem_tabla_a):
         ...
         if data is not None:
             ...
             grafico_linea_dem_a, grafico_tabla_dem_a = tab_graficos_dem_a(tab_linea_dem_a, tab_dem_tabla_a, app_color,tabla_formato_encabezado, tabla_formato_celdas,  df_dem_total_a, df_grafico_histograma_dem_a, df_tabla_desv_dem_a, df_tabla_total_datos_dem_a, nombre_seleccionado_dem_a)
